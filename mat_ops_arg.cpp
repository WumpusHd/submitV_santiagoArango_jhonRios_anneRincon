...
int main(int argc, char** argv) {
    // Suponemos entrada correcta: argv[1]=m, argv[2]=n, argv[3]=p
    int m = std::stoi(std::string(argv[1]));
    int n = std::stoi(std::string(argv[2]));
    int p = std::stoi(std::string(argv[3]));
    ...
    // A(m x n), B_sum(m x n) para suma, B_mul(n x p) para multiplicación
    Matrix A      = rand_matrix(m, n, gen, dist);
    Matrix B_sum  = rand_matrix(m, n, gen, dist);
    Matrix B_mul  = rand_matrix(n, p, gen, dist);
...
// g++ -O3 -std=c++17 mat_ops_arg.cpp -o mat_ops_arg
// ./mat_ops_arg 3 4 5